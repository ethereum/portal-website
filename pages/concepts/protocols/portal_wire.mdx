...
- `TALKRESP` (talk responses). These refer to the request ID being responded to.
...
However, as Portal is a JSON-RPC server (serving Ethereum-related requests like `eth_getBlockNumber`)
...
following table is for the History sub-protocol (`0x500B` in Discovery-terms), and
JSON-RPC methods start with `portal_history*`.

|message|SSZ union message selector|purpose|JSON-RPC|
|-|-|-|-|
|ping|`0x00`|"Are you alive?"|`portal_historyPing`|
|pong|`0x01`|"I'm alive"|None|
|find_nodes|`0x02`|"Give me peers at specific distances x, y & z"|`portal_historyFindNodes`|
|nodes|`0x03`|"Response to findnodes"|None|
|find_content|`0x04`|"I want content x, or peers who might have it"|`portal_historyFindContent`|
|content|`0x05`|"Here is content x, or peers who might have it."|None|
|offer|`0x06`|"I have content x, y & z, would you like any of them?"|`portal_historyOffer`|
|accept|`0x07`|"Yes please, I would like x, y & z"|None|
