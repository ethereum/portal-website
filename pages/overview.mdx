...
The Portal Network is actually several peer-to-peer networks (state, beacon, history, transaction gossip and canonical transaction index networks) that can be accessed using a Portal Network [client](../clients). 
...
There are no intermediaries, and no individual participants need to store large amounts of chain data or handle large numbers of requests.
...
This requires the node operator to dedicate substantial CPU, network bandwidth, hard disk storage and RAM in order to run the node. This
...
are required to serve those requests, and history has demonstrated that few node operators user's do this for altruistic reasons alone.
